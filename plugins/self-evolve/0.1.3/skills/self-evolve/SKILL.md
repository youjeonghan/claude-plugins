...
- 최근 수정 파일 목록
- 세션 대화 요약
- **Session Tool Stats** (transcript jsonl 직접 분석 — 매 실행마다 변화)
...
**Session Tool Stats (transcript jsonl 직접 분석):**

현재 세션의 도구 호출 패턴을 정량 분석한다. workflow-analyzer가 LLM 자가 요약이 아닌 실제 데이터로 비효율을 식별할 수 있도록 한다. 가능하면 반드시 수집하라 — workflow 분석 정확도가 한 자릿수에서 두 자릿수로 올라간다.

```bash
# 현재 세션 jsonl 자동 검출 (Claude Code transcript 위치)
SESSION_DIR="$HOME/.claude/projects/$(pwd | sed 's|/|-|g')"
SESSION_JSONL=$(ls -t "$SESSION_DIR"/*.jsonl 2>/dev/null | head -1)
```

`SESSION_JSONL`을 찾지 못하면(예: Codex CLI, Gemini CLI 등 비-Claude Code 환경) 이 섹션은 `## Session Tool Stats\n(transcript 미발견)` 한 줄만 적고 workflow-analyzer가 git/요약 기반 폴백 분석을 수행하도록 한다.

발견하면 다음 6가지 통계를 추출하여 Context Bundle의 `## Session Tool Stats` 섹션에 주입한다:

| # | 시그널 | jq 추출 (개념) |
|---|--------|---------------|
| 1 | **tool 호출 분포** | `select(.message.content) \| .message.content[]? \| select(.type=="tool_use") \| .name` 카운트 |
| 2 | **반복 Read 파일 (≥2회)** | Read의 `.input.file_path` 카운트 → `awk '$1>=2'` 필터 |
| 3 | **반복 Grep 패턴 (≥2회)** | Grep의 `.input.pattern` 카운트 → `awk '$1>=2'` 필터 |
| 4 | **단일-tool 메시지 비율** | assistant message당 tool_use 블록 개수 → `count==1`인 비율 |
| 5 | **Bash 도구 안티패턴** | Bash의 `.input.command`에서 `grep\|cat\|head\|tail\|find\|ls\|sed\|awk\|rg` 시작 검출 |
| 6 | **도구 실패 카운트** | tool_result 중 `is_error==true` 카운트 + 직후 동일 도구 재호출 페어 검출 |

> **참고 (시도했으나 폐기한 시그널)**: PreToolUse hook reminder 카운트는 강력해 보였으나 Claude Code transcript에 저장되지 않는다(런타임 system-reminder injection만 발생, 영속화 안됨). 사용자가 직접 PostToolUse hook으로 별도 로그 파일을 만들면 누적 가능하지만 사전 설치가 필요하므로 기본 시그널에서는 제외한다.

**구체 jq/grep 예시:**

```bash
# 1. tool 호출 분포
jq -r 'select(.message.content) | .message.content[]? | select(.type=="tool_use") | .name' "$SESSION_JSONL" | sort | uniq -c

# 2. 반복 Read
jq -r 'select(.message.content) | .message.content[]? | select(.type=="tool_use" and .name=="Read") | .input.file_path' "$SESSION_JSONL" | sort | uniq -c | awk '$1>=2'

# 3. 반복 Grep
jq -r 'select(.message.content) | .message.content[]? | select(.type=="tool_use" and .name=="Grep") | .input.pattern' "$SESSION_JSONL" | sort | uniq -c | awk '$1>=2'

# 4. 단일-tool 메시지 비율
jq -r 'select(.type=="assistant") | [.message.content[]? | select(.type=="tool_use")] | length' "$SESSION_JSONL" | awk 'NF{total++; if($1==1) single++} END{if(total>0) printf "%.0f%% (%d/%d)\n", single*100/total, single, total}'

# 5. Bash 안티패턴
jq -r 'select(.message.content) | .message.content[]? | select(.type=="tool_use" and .name=="Bash") | .input.command' "$SESSION_JSONL" | grep -E '^(grep|rg|cat|head|tail|find|ls|sed|awk)( |$)' | head -20

# 6. 도구 실패 카운트 + 재시도 페어
jq -r 'select(.message.content) | .message.content[]? | select(.type=="tool_result" and .is_error==true) | .tool_use_id' "$SESSION_JSONL" | wc -l

# 6-보조: 어떤 도구가 실패했나 (직전 assistant tool_use를 기준으로 매핑)
jq -r '
  select(.type=="assistant") |
  .message.content[]? |
  select(.type=="tool_use") |
  {id: .id, name: .name}
' "$SESSION_JSONL" | jq -s '.' > /tmp/_tool_uses.json
jq -r 'select(.message.content) | .message.content[]? | select(.type=="tool_result" and .is_error==true) | .tool_use_id' "$SESSION_JSONL" | while read id; do
  jq -r ".[] | select(.id==\"$id\") | .name" /tmp/_tool_uses.json
done | sort | uniq -c
```

**참고**: anti-pattern 카탈로그는 `references/anti-patterns.md`에 정리되어 있다. workflow-analyzer가 카탈로그 ID로 매핑할 수 있다.

추출된 데이터는 다음 형식으로 Context Bundle에 추가:

```
## Session Tool Stats
- 총 tool 호출: N (Read=N, Grep=N, Bash=N, Edit=N, ...)
- 반복 Read (≥2회):
  - {file_path}: N회
- 반복 Grep (≥2회):
  - "{pattern}": N회
- 단일-tool 메시지 비율: N% (≥70%면 병렬화 비효율 신호)
- Bash 안티패턴 호출: N건
  - `cat {file}` → Read 권장 (AP-002)
  - `grep {pattern}` → Grep 권장 (AP-001)
- 도구 실패: N건 (도구별: Edit=N, Bash=N, ...)
  - 실패 직후 동일 도구 재시도 페어: N건
```

이 섹션은 캐시 비대상 — 항상 실시간 수집한다.
...
## Session Tool Stats
[6가지 정량 시그널 — Step 2-2 참조. transcript 미발견 시 "(transcript 미발견)"]
