...
Consolidator 결과를 우선순위별로 표시한 뒤, AskUserQuestion으로 적용 항목을 선택받는다.
...
2. ☐ slack 스킬 description에 `with:` 연산자 설명 추가

### Standard (검토 후 적용)
3. ☐ CLAUDE.md에 CPL 상품코드 규칙 섹션 추가
4. ☐ project-memory에 RTI 계산 로직 노트 추가

### Major (신중한 검토 필요)
5. ☐ 새 스킬 생성 제안: notion-image-viewer
6. ☐ 워크플로우 개선: 병렬 검색 패턴 도입
```
...
AskUserQuestion(
    questions=[{
        ...
        "header": "Self-Evolve 개선안",
        "multiSelect": true,
        "options": [
            {"label": "1. permissions.allow에 Bash(npm:*) 추가", "description": "Quick Win"},
            ...
        ]
    }]
)
